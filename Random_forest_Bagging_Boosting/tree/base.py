...
class WeightedDecisionTree:
    ...
    def build_decision_tree(self, X, Y, sample_weights = None, current_depth=0):
        '''Function to build the decision tree recurively'''
        df = pd.DataFrame({'y': Y, 'w': sample_weights})
        grouped = df.groupby(['y']).sum()

        # Leaf node :  --------------------------------------------------------
        # 1. only one output class, 
        # 2. no more attributes to split on, 
        # 3. max depth reached

        if len(df['y'].unique()) == 1:
            return DTree({}, {}, df['y'].unique()[0])
        elif len(X.columns) == 0:
            # return DTree({}, {}, Y.mode()[0])
            if df['y'].dtype == 'category':
                return DTree({}, {}, grouped['w'].idxmax())
            else:
                return DTree({}, {}, np.average(df['y'], weights=df['w']))
        ...
        # Non-leaf node: ------------------------------------------------------
            ...
            if sample_weights is None:
                sample_weights = pd.Series(np.ones(len(Y)))


            bestAttr, split_value = self.findBestAttr(X, Y, sample_weights)
            ...
            if X[bestAttr].dtype != 'category':
                ...
                X_subset_gt = X[X[bestAttr] > split_value]
                Y_subset_gt = Y[X[bestAttr] > split_value]
                wt_subset_le = sample_weights[X[bestAttr] <= split_value]
                wt_subset_gt = sample_weights[X[bestAttr] > split_value]


                subtree1 = self.build_decision_tree(X_subset_le, Y_subset_le, wt_subset_le, current_depth+1)
                cond1 = { (split_value,'<='): Condition('<=', split_value)}
                node = {(split_value,'<='): subtree1}
                tree.addCondition(cond1, node)
                tree.condition_type = 'real'

                subtree2 = self.build_decision_tree(X_subset_gt, Y_subset_gt, wt_subset_gt, current_depth+1)
                cond2 = {(split_value,'>') : Condition('>', split_value)}
                node = {(split_value,'>'): subtree2}
                tree.addCondition(cond2, node)
                tree.condition_type = 'real'
                ...
                for value in X[bestAttr].unique():
                    X_subset = X[X[bestAttr] == value].drop(bestAttr, axis=1)
                    Y_subset = Y[X[bestAttr] == value]
                    wt_subset = sample_weights[X[bestAttr] == value]

                    subtree = self.build_decision_tree(X_subset, Y_subset, wt_subset, current_depth+1)
                    cond = {(value,'='): Condition('==', value)}
                    node = {(value, '='): subtree}
                    tree.addCondition(cond, node)
                    tree.condition_type = 'categorical'
    ...
    def findBestAttr(self, X: pd.DataFrame, y: pd.Series, wt: pd.Series = None) ->(str, float):

        if wt is None:
            wt = pd.Series(np.ones(len(y)))

        if y.dtypes == 'object' or y.dtypes == 'category':
            if self.criterion == "information_gain":     
                bestAttr = None
                bestGain = -1* (math.log2(wt.sum())+1)
                split_value = None           
                for attr in X.columns:
                    gain, split = information_gain(y, X[attr], wt)
                    if gain > bestGain:
                        bestGain = gain
                        bestAttr = attr  
                        if split:
                ...
                split_value = None
                for attr in X.columns:
                    gain, split = avg_gini_coefficient(y, X[attr], wt)
                    if gain < bestGini:
                        bestGini = gain
                        bestAttr = attr
                        if split:
            ...
            split_value = None
            for attr in X.columns:
                gain, split = reduction_in_variance(y, X[attr], wt)
                if gain > bestGain:
                    bestGain = gain
                    bestAttr = attr
                    if split:
    ...
    def fit(self, X: pd.DataFrame, y: pd.Series, sample_weights: pd.Series = None) -> None:
        X_copy = X.copy()
        y_copy = y.copy()
        if sample_weights is None:
            sample_weights = pd.Series(np.ones(len(y)))
        # y_weighted = pd.DataFrame({'y':y,'w':sample_weights})
        tree = self.build_decision_tree(X_copy, y_copy, sample_weights)
        self.root = tree
